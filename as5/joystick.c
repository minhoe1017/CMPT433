...
{
	isButtonPressed = readButtonWithStarteWare();
}

/*
** The main function for joystick. 
*/
void joystickMain()
{
	uint32_t counter = 0;
...
//		_Bool isButtonPressed = readButtonWithBitTwiddling();
		...
		counter++;
		if (isButtonPressed) {
//			ConsoleUtilsPrintf("Changing to Toggle Mode");
			if (lastButtonState) 							// if currentMode is bounce 
				barPattern();								// change to bar 
			else											// if currentMode is bar
				bouncePattern();				// change to bounce
			// Hit the watchdog
