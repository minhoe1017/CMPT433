...
/*void swapContent(double *d1, double *d2)
{
	double tmp = *d2;
	*d2 = *d1;
	*d1 = tmp; 			// BUG was here: it was only swapping tmp copies rather than the real pointers 
}*/
void swapContent(double **d1, double **d2)
{
	double *tmp = *d2;
	*d2 = *d1;
	*d1 = tmp;			// should be able to accept double pointers
}
...
int main()
{
	...
	swapContent(&source, &target); 				//accepting address as using double pointer (not pointer itself)
